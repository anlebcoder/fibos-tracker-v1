...
module.exports = db => {
	...
	Transactions.updateStatus = function(bn) {
		let rs = db.driver.execQuerySync("UPDATE `transactions` set status = 'yes', updatedAt = ? where block_num <= ? and status = 'no';", [new Date(), bn]);
		return rs.affected;
	}
